...
export function createHttpApi(opts: ClientOptions = {}): Api {
  ...
  return {
    ...
    setupStatus: () => call<SetupStatus>('GET', '/v1/auth/setup'),
    setup: (body) => call<LoginOK>('POST', '/v1/auth/setup', body),
    ...
    organisation: () => call<{ name: string }>('GET', '/v1/admin/settings/organisation'),
    setOrganisation: (name) => call<{ name: string }>('PUT', '/v1/admin/settings/organisation', { name }),
    ...
    updateTile: (id, body) => call<Tile>('PUT', `/v1/admin/portal/tiles/${encodeURIComponent(id)}`, body),
    deleteTile: (id) => call<void>('DELETE', `/v1/admin/portal/tiles/${encodeURIComponent(id)}`),
