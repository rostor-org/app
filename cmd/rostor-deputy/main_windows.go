...
// rostor-deputy is the Rostor device deputy for Windows (contract: docs/contracts/
// windows-logon.md). It runs as the RostorDeputy service and brokers logon
...
import (
	...
	"rostor.org/app/cmd/rostor-deputy/internal/broker"
	...
	"rostor.org/app/cmd/rostor-deputy/internal/pipe"
...
commands:
  run               run the deputy (as a service when started by SCM, else in the foreground)
  ...
  install-service   register and start the RostorDeputy service
                    flags: [--mock-core]
  uninstall-service stop and remove the RostorDeputy service
  pipe-test         send one request to the deputy pipe and print the reply
  ...
  version           print the deputy version
`
...
func main() {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "rostor-deputy:", err)
		os.Exit(1)
	}
}
...
func cmdRun(args []string) error {
	...
	logger.Printf("rostor-deputy %s listening on %s (mock-core=%v, enrolled=%v)", version, paths.PipeName, *mock, b.Verifier != nil && !*mock)
...
// deputy.not_enrolled on logon rather than leave the pipe dead.
func buildBroker(logger *log.Logger, mock bool) (*broker.Broker, error) {
	...
	if errors.Is(err, os.ErrNotExist) {
		logger.Printf("not enrolled (%s missing); logon will answer deputy.not_enrolled", paths.DeputyJSON)
		return b, nil
	}
	if err != nil {
		return nil, err
...
func agentFiles() trust.Files {
	return trust.Files{DeputyJSON: paths.DeputyJSON, DeviceKey: paths.DeviceKey, DeviceCert: paths.DeviceCert, CACert: paths.CACert}
}
...
func cmdEnroll(args []string) error {
	...
	cfg, err := enroll.Run(context.Background(), enroll.Options{
		...
		Files: enroll.Files{
			DeputyJSON: paths.DeputyJSON, DeviceKey: paths.DeviceKey,
	...
	fmt.Println("restart the RostorDeputy service to pick up the enrollment")
	return nil
}
...
func cmdRenew(args []string) error {
	...
	fmt.Println("the RostorDeputy service picks the new certificate up at its next trust check (within 10 minutes) or on restart")
	return nil
}
