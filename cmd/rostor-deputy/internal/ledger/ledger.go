// Package ledger records which local Windows accounts the deputy created
// (contract §3). The ledger is authoritative: the deputy only ever modifies an
...
// Entry is one deputy-created account.
type Entry struct {
...
// Ledger is a small JSON file guarded by a mutex; the deputy handles one logon
...
// Has reports whether username was created by the deputy.
...
func (l *Ledger) saveLocked() error {
	...
	// which would silently orphan accounts the deputy created.
	tmp := l.path + ".tmp"
