...
// Config is the persisted deputy.json (contract §4). Resource is an addition:
// the enrollment response names the workstation resource and the deputy must
...
type Files struct {
	DeputyJSON, DeviceKey, DeviceCert, CACert string
}
...
func Run(ctx context.Context, o Options) (*Config, error) {
	...
	if _, err := os.Stat(o.Files.DeputyJSON); err == nil {
		return nil, fmt.Errorf("enroll: %s already exists; remove it to re-enroll", o.Files.DeputyJSON)
	}
	...
	if err := os.MkdirAll(filepath.Dir(o.Files.DeputyJSON), 0o755); err != nil {
		return nil, err
	}
	...
	if err := os.WriteFile(o.Files.DeputyJSON, cfgJSON, 0o644); err != nil {
		return nil, err
	}
	return cfg, nil
}
...
func LoadConfig(path string) (*Config, error) {
	...
	if err := json.Unmarshal(b, &c); err != nil {
		return nil, fmt.Errorf("deputy.json: %w", err)
	}
	if c.CoreURL == "" {
		return nil, errors.New("deputy.json: core_url is empty")
	}
	return &c, nil
}
...
// SaveConfig rewrites deputy.json atomically (write path.new, rename).
