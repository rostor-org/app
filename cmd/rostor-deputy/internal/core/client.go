...
// so the deputy must answer before that with a rendered deputy.core_unreachable.
...
// deputy.core_unreachable without inspecting error strings.
...
func (c *Client) Posture(ctx context.Context, osVersion string) error {
	body := map[string]string{"deputy_version": c.AgentVersion, "os_version": osVersion}
