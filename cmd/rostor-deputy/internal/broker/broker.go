// Package broker implements the deputy side of the pipe protocol: it turns a
...
import (
	...
	"rostor.org/app/cmd/rostor-deputy/internal/catalog"
...
type Broker struct {
	// Verifier is nil when the deputy is not enrolled; every logon then
	// answers deputy.not_enrolled rather than crashing the lock screen path.
...
func (b *Broker) logon(ctx context.Context, locale string, req pipeproto.Request) pipeproto.Reply {
	...
	if b.Verifier == nil {
		return b.fail(locale, "deputy.not_enrolled", "")
	}
	started := time.Now()
	...
	if err != nil {
		...
		if errors.Is(err, context.DeadlineExceeded) {
			return b.fail(locale, "deputy.timeout", "")
		}
		return b.fail(locale, "deputy.core_unreachable", "")
	}
	...
	// username the deputy could act on.
...
// fail builds a denial. Core-rendered text wins; deputy-local and unrendered
