...
func TestInspectReport(t *testing.T) {
	...
	if err := enroll.SaveConfig(f.AgentJSON, &enroll.Config{CoreURL: "https://x", TrustVersion: "abc"}); err != nil {
		t.Fatal(err)
	}
	r := Inspect(f)
...
func enrolled(t *testing.T, fc *fakeCore, ca *testCA, ttl time.Duration) (Files, *enroll.Config) {
	...
	if err := enroll.SaveConfig(f.AgentJSON, cfg); err != nil {
		t.Fatal(err)
	}
	return f, cfg
}
...
func TestManagerRotationRenewsThroughStagedPair(t *testing.T) {
	...
	saved, _ := enroll.LoadConfig(f.AgentJSON)
