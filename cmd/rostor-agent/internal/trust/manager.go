...
type Manager struct {
	Client *core.Client
	Files  Files
	// Config is agent.json; Manager owns trust_version in it.
...
// Errors are returned for the log; the agent keeps running on whatever pair
...
// Renew forces a renewal regardless of expiry (`rostor-agent renew --now`).
...
func (m *Manager) applyBundle(version string, pems []string) error {
	...
	if err := enroll.SaveConfig(m.Files.AgentJSON, m.Config); err != nil {
...
// certificate in memory, e.g. after `rostor-agent renew --now` ran beside
// the service.
