...
type Server struct {
	...
	// 10 s so the agent, not the credprov, produces the timeout text.
	PerRequest time.Duration

	ln net.Listener
}

// Listen creates the pipe. It fails if another agent already owns it.
...
func (s *Server) serveConn(conn net.Conn) {
	...
	if err != nil {
		...
		_ = pipeproto.Write(conn, pipeproto.Reply{OK: false, Code: "agent.bad_request"})
		return
	}
	started := time.Now()
