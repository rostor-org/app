...
// `ui` or `logon` request into a reply by consulting core and the local
// account manager. It holds all the decision-to-action mapping from contract
...
func (b *Broker) logon(ctx context.Context, locale string, req pipeproto.Request) pipeproto.Reply {
	...
	resp, err := b.Verifier.Verify(ctx, core.VerifyRequest{
		Credential: core.Credential{Type: "password", Identifier: req.Identifier, Secret: req.Secret},
		Action:     "logon",
		Resource:   b.Resource,
		Locale:     locale,
	})
	if err != nil {
		b.logf("logon %q: core error after %s: %v", req.Identifier, time.Since(started).Round(time.Millisecond), err)
	...
	if resp.Decision == "ALLOW" {
		return b.allow(locale, req.Identifier, resp)
	}
	...
	b.logf("logon %q: DENY %s", req.Identifier, code)
	// Suspension and lifecycle denials also close the door locally so a
	// cached Windows credential cannot outlive the directory decision.
	if code == "principal.suspended" || code == "principal.not_active" {
