// Package catalog is the agent's bundled string catalog for agent-local codes
// and the credential provider's UI labels (contract §2). Core renders text for
// its own codes; this catalog only covers what the agent produces on its own
// (for example when core is unreachable). Nothing in the agent formats
...
// Message renders an agent-local code. Unknown codes return the code itself:
