...
| Endpoint | Returns |
|---|---|
| `GET /v1/admin/summary` | counts: active, suspended, applicants, service accounts, groups, grants, devices by lifecycle, oldest snapshot age (null for now) |
| `GET /v1/admin/users` | items: `{id, username, display_name, state, groups:[{id,name}], methods:[{method,assurance}], last_sign_in:{ts,resource}}` |
| `GET /v1/admin/users/{id}` | the item plus `bindings:[{id,method,properties,label,created_at,last_used_at,state}]`, `effective_security:{assurance,bindings,recovery_paths}`, `recent:[audit rows]` |
| `GET /v1/admin/groups` | items: `{id,name,display_name,kind,member_count,grant_count}` |
| `GET /v1/admin/groups/{name}` | plus `members:[{kind,id,username|name,display_name}]`, `grants:[…]` |
...
| `GET /v1/admin/audit?limit=&before=<seq>&q=` | `{"items":[{seq,ts,actor,action,target,credential_type,assurance,outcome,detail,correlation_id}],"head":{seq}}` |
...
| `GET /v1/admin/system` | `{version, tenant:{id,name}, listeners, db:{size_bytes}, uptime_seconds, ca:{subject,not_after}, release_key_fingerprint, profile:"standard"}` |
