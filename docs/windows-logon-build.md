...
| path | what | builds on |
|---|---|---|
| `cmd/rostor-deputy/` | Go Windows service `RostorDeputy` + CLI (`enroll`, `install-service`, `uninstall-service`, `pipe-test`, `trust`, `renew`, `run`) | macOS/Linux cross-compile |
...
Deputy (from the repo root, any OS):

```sh
GOOS=windows GOARCH=amd64 go build -o rostor-deputy.exe ./cmd/rostor-deputy
go vet ./cmd/rostor-deputy/...            # also passes with GOOS=windows
go test ./cmd/rostor-deputy/...           # pure-logic tests run on macOS
```
...
Every GitHub release carries `rostor-windows-amd64.zip` (deputy, credential
...
That one command enrolls the workstation (`rostor-deputy enroll`, run
before the service starts), installs the `RostorDeputy` service, sets the
...
enrollment call has no CA to verify against yet, so it runs with the deputy's
...
Re-running is safe: when `C:\ProgramData\Rostor\deputy.json` already exists
...
.\install.ps1 -SkipCredProv   # deputy + service only
...
`install.ps1` refuses to register the credential provider unless the deputy
...
job (Windows runner, MSVC), cross-builds the deputy with `setup-go`, zips the
...
Copy `rostor-deputy.exe`, `RostorCredProv.dll`, `install.ps1`, `uninstall.ps1`
...
followed by `Restart-Service RostorDeputy`):

```powershell
& 'C:\Program Files\Rostor\rostor-deputy.exe' enroll --core-url https://core:8443 --token <enrollment token> [--ca-file core-ca.pem | --insecure]
```
...
The deputy pins the core CA bundle (`ca.crt`, all active CAs concatenated)
and holds a 90-day device certificate. Both are kept current by the
`RostorDeputy` service itself, per `docs/contracts/console-api.md`
("Certificates and trust"):

- At service start and every 10 minutes the deputy calls
  `GET /v1/devices/self/trust`. When `version` differs from `trust_version`
  in `deputy.json` it rewrites `ca.crt` (write `ca.crt.new`, rename) and
...
- A logon `Verify` that fails in the TLS handshake (the usual sign of a CA
  ...
  before the credprov sees `deputy.core_unreachable`.

Operator commands (elevated):

```powershell
rostor-deputy trust        # pinned CA fingerprints, device cert issuer + expiry, last trust version; no network
rostor-deputy renew --now  # force a renewal; the service reloads within 10 minutes or on restart
```

## Debugging

- Deputy log: `C:\ProgramData\Rostor\logs\deputy.log`
- Credprov log: `C:\ProgramData\Rostor\logs\credprov.log` (written by LogonUI as SYSTEM)
- Talk to the pipe as an elevated admin:
  `rostor-deputy pipe-test --op ui`,
  `rostor-deputy pipe-test --op logon --identifier dan --secret ...`,
  `rostor-deputy pipe-test --op logon --badge 5555555555 [--pin 2468]`
- Foreground deputy: stop the service, then `rostor-deputy run [--mock-core]`.

## Verified so far (2026-09-22, Windows 10 Pro N 19045 VM, deputy in `--mock-core`)

- `GOOS=windows GOARCH=amd64 go build ./cmd/rostor-deputy`, `go vet` (darwin and
  windows), `go test ./cmd/rostor-deputy/...` all pass.
- `install.ps1 -MockCore` installs and starts `RostorDeputy` (LocalSystem,
...
- `RostorCredProv.dll` builds cleanly with `/W4 /WX` (x64); exports
  ...
  `GetSerialization` against the live deputy and gets
  ...
  `CPGSR_NO_CREDENTIAL_FINISHED` + `CPSI_ERROR` + the deputy's text.
...
## Verified 2026-09-24 (badge + PIN, same VM, deputy in `--mock-core`)
...
- `cp_test.exe <dll> --badge <number> <pin|-> expect-ok|expect-deny|expect-pin`
  ...
  yields `CPGSR_NO_CREDENTIAL_NOT_FINISHED` + the deputy's prompt, the secret
...
400 to a badge presentation at the time of writing, which the deputy reports as
`deputy.core_unreachable`), `CPUS_UNLOCK_WORKSTATION`.
...
deputy fetches `GET /v1/devices/self/policy` and keeps the last successful
answer; a failed fetch is logged and the previous value stays in force,
and before the first answer the deputy behaves as if the default method
